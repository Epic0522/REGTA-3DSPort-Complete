...
#ifdef _3DS
			const float occupantDrawDist = m_pMyVehicle->IsBoat() ? 36.0f : 28.0f;
#else
			const float occupantDrawDist = (m_pMyVehicle->IsBoat() ? 40.0f : 25.0f) * TheCamera.LODDistMultiplier;
#endif
			if (camDistSq > SQR(occupantDrawDist))
				return;
		}
	}
