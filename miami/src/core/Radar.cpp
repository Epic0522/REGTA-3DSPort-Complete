...
#ifdef ENABLE_3DS_BOTTOM_RADAR
		// The original minimum range was tuned for a tiny 94x76 HUD radar.
		// On a 200+ pixel display it over-magnifies the low-resolution tiles.
		if (m_bDrawingBottomScreen)
...
{
#if !defined(_3DS_RADAR_NO_MASK_DIAGNOSTIC)
	#ifdef ENABLE_3DS_BOTTOM_RADAR
	if (!m_bDrawingBottomScreen)
	#endif
	DrawRadarMask();
...
{ 
	...
	RwRenderStateSet(rwRENDERSTATEZTESTENABLE, (void*)FALSE);
	#ifdef ENABLE_3DS_BOTTOM_RADAR
	RwRenderStateSet(rwRENDERSTATEZWRITEENABLE, (void*)(m_bDrawingBottomScreen ? FALSE : TRUE));
	#else
...
#ifdef ENABLE_3DS_BOTTOM_RADAR
	if (m_bDrawingBottomScreen) {
		// The 3DS depth-mask path rejects the entire map. On the dedicated
		// lower camera, draw opaque corner fans over the already-rendered map
		// instead. Blips are rendered afterwards and remain whole on the rim.
		RwRenderStateSet(rwRENDERSTATESRCBLEND, (void*)rwBLENDSRCALPHA);
		RwRenderStateSet(rwRENDERSTATEDESTBLEND, (void*)rwBLENDINVSRCALPHA);
	} else
#endif
	...
	for (int i = 0; i < 4; i++) {
		...
		if (m_bDrawingBottomScreen) {
			CVector2D uv[8];
			for(int j = 0; j < 8; j++)
				uv[j] = CVector2D(0.0f, 0.0f);
			CSprite2d::SetVertices(8, (float *)out, (float *)uv, CRGBA(8, 10, 16, 255));
		} else
		#endif
...
#ifdef ENABLE_3DS_BOTTOM_RADAR
	if(m_bDrawingBottomScreen) {
		// Keep the centre fixed at (120,120), but place every off-radar blip
		// directly on the inset rectangular screen edge.  Merely clamping points
		// that had already crossed that rectangle left the region between the
		// stock unit circle and the rectangle untouched, which made freshly
		// created mission blips form a second circular track inside the square.
...
void CRadar::TransformRadarPointToScreenSpace(CVector2D &out, const CVector2D &in)
{
	#ifdef ENABLE_3DS_BOTTOM_RADAR
	if (m_bDrawingBottomScreen) {
		// 216-pixel circle centred in the lower display's left 240 pixels.
