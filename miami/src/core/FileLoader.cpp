...
/* The initial streamed COL pass used to scan all 6500 model slots for every
 * collision model name.  VC has enough COL records for those case-insensitive
 * string scans to dominate this loading stage on the 3DS CPU. */
static const int COL_MODEL_NAME_INDEX_SIZE = 8192;
static uint16 colModelNameIndex[COL_MODEL_NAME_INDEX_SIZE];
static bool colModelNameIndexBuilt;

static uint32
HashColModelName(const char *name)
{
	uint32 hash = 2166136261u;
	for(int i = 0; i < MAX_MODEL_NAME && name[i]; i++){
		hash ^= (uint8)tolower((uint8)name[i]);
		hash *= 16777619u;
	}
	return hash;
}
...
static void
BuildColModelNameIndex(void)
{
	memset(colModelNameIndex, 0, sizeof(colModelNameIndex));
	for(int id = 0; id < MODELINFOSIZE; id++){
		CBaseModelInfo *mi = CModelInfo::GetModelInfo(id);
		if(mi == nil || mi->GetModelName()[0] == '\0')
			continue;

		uint32 slot = HashColModelName(mi->GetModelName()) & (COL_MODEL_NAME_INDEX_SIZE - 1);
		while(colModelNameIndex[slot] != 0)
			slot = (slot + 1) & (COL_MODEL_NAME_INDEX_SIZE - 1);
		colModelNameIndex[slot] = id + 1;
	}
	colModelNameIndexBuilt = true;
}

static CBaseModelInfo*
FindColModelInfo(const char *name, int *id)
{
	if(!colModelNameIndexBuilt)
		BuildColModelNameIndex();

	uint32 slot = HashColModelName(name) & (COL_MODEL_NAME_INDEX_SIZE - 1);
	for(int probes = 0; probes < COL_MODEL_NAME_INDEX_SIZE; probes++){
		uint16 entry = colModelNameIndex[slot];
		if(entry == 0)
			break;
		int modelId = entry - 1;
		CBaseModelInfo *mi = CModelInfo::GetModelInfo(modelId);
		if(mi && !CGeneral::faststricmp(mi->GetModelName(), name)){
			if(id)
				*id = modelId;
			return mi;
		}
	...
	/* Keep correctness if a model name was registered after the one-time index
	 * build (for example by a developer-only streaming path). */
	return CModelInfo::GetModelInfo(name, id);
}
...
CFileLoader::LoadCollisionFile(const char *filename, uint8 colSlot)
{
	...
	while(CFileMgr::Read(fd, (char*)&header, sizeof(header))){
		...
		mi = FindColModelInfo(modelname, nil);
		if(mi){
...
CFileLoader::LoadCollisionFileFirstTime(uint8 *buffer, uint32 size, uint8 colSlot)
{
	...
	while(size > 8){
		...
		mi = FindColModelInfo(modelname, &modelIndex);
		if(mi){
			CColStore::IncludeModelIndex(colSlot, modelIndex);
			CColModel *model = new CColModel;
			model->level = colSlot;
			LoadCollisionModel(buffer+32, *model, modelname);
			mi->SetColModel(model, true);
		}else{
			debug("colmodel %s can't find a modelinfo\n", modelname);
		}
...
CFileLoader::LoadCollisionFile(uint8 *buffer, uint32 size, uint8 colSlot)
{
	...
	while(size > 8){
		...
		if(mi){
			if(mi->GetColModel()){
				LoadCollisionModel(buffer+32, *mi->GetColModel(), modelname);
			}else{
				CColModel *model = new CColModel;
				model->level = colSlot;
				LoadCollisionModel(buffer+32, *model, modelname);
