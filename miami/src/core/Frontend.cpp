...
#ifdef _3DS
	if (instantPause) {
		// This is the normal settled geometry generated by Vice City's pause
		// home page.  Initialise it before the first visible frame rather than
		// showing the uncropped background texture first.
...
#ifdef _3DS
		if (strcmp(text, "3DS_MPH") == 0) {
			static wchar mapHelp[80];
			static bool mapHelpReady = false;
			if (!mapHelpReady) {
				AsciiToUnicode("C-PAD: MOVE  ZR/R: ZOOM  L: LEGEND  Y: MARKER  B: BACK", mapHelp);
				mapHelpReady = true;
			}
			CFont::PrintString(SCREEN_STRETCH_FROM_RIGHT(HELPER_TEXT_RIGHT_MARGIN), SCREEN_SCALE_FROM_BOTTOM(HELPER_TEXT_BOTTOM_MARGIN), mapHelp);
		} else
#endif
...
CMenuManager::DrawBackground(bool transitionCall)
{
	if (!m_bSpritesLoaded) {
		...
		if (!m_bGameNotLoaded && m_nCurrScreen == MENUPAGE_PAUSE_MENU && gPauseBackgroundResident3DS) {
			// Re-enter the original VC renderer with the resident FRONTEN1
			// sprites.  It applies the changing quadrilateral, black masks, logo,
			// option text and highlight exactly as the normal full menu does.
			m_bSpritesLoaded = true;
			DrawBackground(transitionCall);
...
CMenuManager::SwitchMenuOnAndOff()
{
	if (!TheCamera.m_WideScreenOn) {
		bool startPressed = CPad::GetPad(0)->GetStartJustDown();
#ifdef _3DS
		if (gPauseWaitForStartRelease3DS) {
			if (CPad::GetPad(0)->GetStart())
				startPressed = false;
			else
				gPauseWaitForStartRelease3DS = false;
		}
#endif

		// Reminder: You need REGISTER_START_BUTTON defined to make it work.
		if ((startPressed || CPad::GetPad(0)->GetEscapeJustDown())
...
#ifdef _3DS
			if (m_bMenuActive && startPressed)
				gPauseWaitForStartRelease3DS = true;
#endif
