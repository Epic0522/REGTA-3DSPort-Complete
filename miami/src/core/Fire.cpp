...
#ifdef _3DS
		const uint32 baseFlameInterval = 120;
#else
		const uint32 baseFlameInterval = 80;
#endif
		m_nNextTimeToAddFlames = CTimer::GetTimeInMilliseconds() + (m_fWaterExtinguishCountdown < 0.3f ? 400 : (m_fWaterExtinguishCountdown < 0.7f ? 200 : baseFlameInterval));
		firePos = m_vecPos;
