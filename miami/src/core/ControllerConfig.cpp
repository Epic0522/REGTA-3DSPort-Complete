...
#ifdef _3DS
/* The 3DS build has no readable controller-icon atlas at its native
 * resolution.  Use the labels printed on the console and plain ASCII names
 * for both sticks so every ~k~ gameplay hint remains legible. */
#undef UP
#undef DOWN
#undef LEFT
#undef RIGHT
...
{
#ifdef _3DS
	/* 3DS input is written directly into CPad rather than passing through the
	 * configurable desktop joystick layer.  Resolve help tokens from the fixed
	 * hardware map, independent of tiny/uneditable desktop controller settings. */
	wchar wstr[24];
