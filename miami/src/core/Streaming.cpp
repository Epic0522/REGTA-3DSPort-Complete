...
#ifdef RW_3DS
static bool
ReleaseOneStreamedResourceForTexture(size_t)
{
	/* Called synchronously by librw while a new raster is being allocated.
	 * The item currently being read is not on the loaded LRU list, so this can
	 * only discard an older, unreferenced resource that is safe to reload. */
...
#ifdef RW_3DS
	rw::c3d::TexSetMemoryPressureCallback(ReleaseOneStreamedResourceForTexture);
#endif
...
CStreaming::LoadAllRequestedModels(bool priority)
{
	...
	const int totalRequests = Max(priority ? ms_numPriorityRequests : ms_numModelsRequested, 1);
	...
	auto takeNextRequest = [&](PendingLoad &load, int channel) {
		...
		while(ms_endRequestedList.m_prev != &ms_startRequestedList && numRequests > 0){
			...
			if(!ms_aInfoForModel[streamId].GetCdPosnAndSize(load.posn, load.size)){
				ms_aInfoForModel[streamId].m_loadState = STREAMSTATE_LOADED;
				completedRequests++;
				UpdateLoadingScreenProgressRange((float)completedRequests / totalRequests);
				continue;
			}
	...
	auto convertLoad = [&](PendingLoad &load) {
		...
		if(ms_aInfoForModel[load.streamId].m_loadState == STREAMSTATE_STARTED)
	...
	PendingLoad current = {-1, 0, 0, 0, false, false};
	PendingLoad next = {-1, 1, 0, 0, false, false};
	...
	while(current.valid){
		...
		if(haveNext){
			...
			current = next;
			next = {-1, 1 - current.channel, 0, 0, false, false};
		}
	}
