	...
	for (int i = 0; i < ARRAY_SIZE(gaExplosion); i++) {
		...
		if (explosion.m_nParticlesExpireTime != 0) {
			...
			case EXPLOSION_BARREL:
				...
				break;
			case EXPLOSION_MOLOTOV:
				// Damage and propagation remain unchanged; stagger the two full
				// population scans and bound only the visual burst on 3DS.
#ifdef _3DS
				...
				if (explosion.m_nIteration < 10) {
					...
					int numParticles = 5 * ff;
#ifdef _3DS
					numParticles = Min(numParticles, 8);
#endif
			...
			case EXPLOSION_BOAT:
				if (someTime >= 3500) {
					if (explosion.m_pVictimEntity != nil) {
						...
						if (CTimer::GetTimeInMilliseconds() > explosion.m_fStartTime) {
						#ifdef _3DS
							explosion.m_fStartTime = CTimer::GetTimeInMilliseconds() + 220 + (CGeneral::GetRandomNumber() & 0x7F);
						#else
							...
							int numBursts = (CGeneral::GetRandomNumber() & 1) + 1;
#ifdef _3DS
							numBursts = 1;
#endif
