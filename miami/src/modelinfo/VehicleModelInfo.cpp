...
#ifdef _3DS
/* Admiral's rear plate is not a normal plates* material.  It is a four-vertex
 * patch inside the admiral868bit128 atlas, only 0.004 units in front of the
 * rear body.  The 3DS depth buffer cannot keep those surfaces separated, so
 * move only that patch outward and leave the bumper/body geometry untouched. */
static RpAtomic*
FixAdmiralPlateDepthCB(RpAtomic *atomic, void*)
{
	if(strcmp(GetFrameNodeName(RpAtomicGetFrame(atomic)), "chassis_hi") != 0)
		return atomic;

	RpGeometry *geometry = RpAtomicGetGeometry(atomic);
	bool isAdmiralAtlas = false;
	for(int32 i = 0; i < geometry->matList.numMaterials; i++){
		RwTexture *texture = RpMaterialGetTexture(geometry->matList.materials[i]);
		if(texture && strcmp(RwTextureGetName(texture), "admiral868bit128") == 0){
			isAdmiralAtlas = true;
			break;
		}
	}
	...
	RwV3d *vertices = RpMorphTargetGetVertices(RpGeometryGetMorphTarget(geometry, 0));
	int32 plateVertices[4];
	int32 count = 0;
	for(int32 i = 0; i < RpGeometryGetNumVertices(geometry); i++){
		const RwV3d &v = vertices[i];
		if(v.x > -0.20f && v.x < 0.20f &&
		   v.y < -2.81f && v.y > -2.83f &&
		   v.z > -0.18f && v.z < 0.05f){
			if(count < 4)
				plateVertices[count] = i;
			count++;
		}
	}
	/* Refuse to touch an unexpected model revision instead of shifting nearby
	 * body vertices.  The shipped Admiral has exactly these four plate corners. */
	if(count == 4){
		RpGeometryLock(geometry, rpGEOMETRYLOCKVERTICES);
		for(int32 i = 0; i < 4; i++)
			vertices[plateVertices[i]].y -= 0.012f;
...
#ifdef _3DS
	RpClumpForAllAtomics(m_clump, FixAdmiralPlateDepthCB, nil);
#endif
...
CVehicleModelInfo::SetEnvironmentMapCB(RpAtomic *atomic, void *data)
{
	...
	/* Some VC vehicles (notably the Rhino) have atomics without an explicit
	 * MatFX material.  On 3DS those atomics would fall back to the default
	 * per-vertex lighting pipeline and bring the dark triangular facets back.
	 * The 3DS MatFX pipeline has a safe texture-only fallback for exactly this
	 * case, so route every vehicle atomic through it. */
#ifdef RW_3DS
