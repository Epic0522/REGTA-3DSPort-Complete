...
CAutomobile::dmgDrawCarCollidingParticles(const CVector &pos, float amount)
{
...
#ifdef _3DS
	// Keep physical collision response untouched.  Only budget the visual burst
	// generated after ambient traffic contacts; the player keeps full feedback.
	static uint32 collisionBudgetFrame = UINT32_MAX;
	static uint8 ambientCollisionBursts;
	const uint32 currentFrame = CTimer::GetFrameCounter();
	if(collisionBudgetFrame != currentFrame){
		collisionBudgetFrame = currentFrame;
		ambientCollisionBursts = 0;
	}
	if(this != FindPlayerVehicle()){
		if(ambientCollisionBursts >= 4)
			return;
		ambientCollisionBursts++;
	}
#endif
	...
	int numSparks = ((n+4)&0x1F);
#ifdef _3DS
	numSparks = Min(numSparks, 6);
#endif
