...
void CBulletTraces::AddTrace(CVector* start, CVector* end, float thickness, uint32 lifeTime, uint8 visibility)
{
...
#ifdef _3DS
	/* A full VC trace is three textured Im3D submissions and the original can
	...
	nextSlot = 0;
	for (int i = 0; nextSlot < activeTraceLimit && aTraces[i].m_bInUse; i++)
		nextSlot++;
	if (nextSlot < activeTraceLimit) {
...
#ifdef _3DS
		/* The centre cross already represents the complete tracer.  The following
		 * two submissions only add fading end caps, tripling its per-frame cost. */
		continue;
#endif
