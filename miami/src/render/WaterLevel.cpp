...
// Use most of the 106-unit small-water transition zone to move from the
// transparent wavy layer to the opaque flat layer. The old 20-unit ramp was
// also accidentally quantised to 0/1 below, making its boundary a hard band.
float fFlatWaterBlendRange    = 1.0f / 96.0f;
float fStartBlendDistanceAdd  = 0.0f;
...
#ifdef _3DS
		/* Keep near dynamic water on the same stable colour path as the buffered
		 * far sectors; the default byte-prelight route can saturate on PICA200. */
...
#else
				/* The close-range VC mask is a second 33x33 dynamic water mesh
				 * rendered through MatFX.  On 3DS its per-frame reinstance/MatFX
				 * pass contaminates vehicle rendering whenever this nearby-water
				 * branch becomes active; leaving the water block immediately clears
				 * the symptom.  Keep the underlying wavy/transparent water and omit
				 * only this reflective overlay. */
				(void)fMaskX;
				(void)fMaskY;
				(void)fWaterZ;
				(void)fSectorX;
	...
	if (!WavesCalculatedThisFrame)
	{
		...
		for ( int32 i = 0; i < 17; i++ )
		{
			for ( int32 j = 0; j < 17; j++ )
			{
				...
				RwRGBAAssign(wavyPreLight, &white);
#else
...
#ifdef _3DS
		RpMaterialSetColor(RpGeometryGetMaterial(wavyGeometry, 0), &color);
#endif
	}
		...
		float fDrawDist)
{
	...
	float fAlphaBlendMulA
			= Min(Max(fFlatWaterBlendRange * (sqrt(fDistStartX + fDistStartY) - fBlendDrawDist), 0.0f), 1.0f);
	float fAlphaBlendMulB
			= Min(Max(fFlatWaterBlendRange * (sqrt(fDistStartX + fDistEndY  ) - fBlendDrawDist), 0.0f), 1.0f);
	float fAlphaBlendMulC
			= Min(Max(fFlatWaterBlendRange * (sqrt(fDistEndX   + fDistEndY  ) - fBlendDrawDist), 0.0f), 1.0f);
	float fAlphaBlendMulD
			= Min(Max(fFlatWaterBlendRange * (sqrt(fDistEndX   + fDistStartY) - fBlendDrawDist), 0.0f), 1.0f);
	...
	RwIm3DVertexSetRGBA(&TempBufferRenderVertices[vidx + 0], color.red, color.green, color.blue,
		(uint8)(colorTrans.alpha + (color.alpha - colorTrans.alpha) * fAlphaBlendMulA));
	
	RwIm3DVertexSetPos  (&TempBufferRenderVertices[vidx + 1], fX, fY + SMALL_SECTOR_SIZE, fZ - _fWaterZOffset);
	RwIm3DVertexSetU    (&TempBufferRenderVertices[vidx + 1], TEXTURE_ADDU);
	RwIm3DVertexSetV    (&TempBufferRenderVertices[vidx + 1], TEXTURE_ADDV + 1.0f);
	RwIm3DVertexSetRGBA(&TempBufferRenderVertices[vidx + 1], color.red, color.green, color.blue,
		(uint8)(colorTrans.alpha + (color.alpha - colorTrans.alpha) * fAlphaBlendMulB));
	
	RwIm3DVertexSetPos  (&TempBufferRenderVertices[vidx + 2], fX + SMALL_SECTOR_SIZE, fY + SMALL_SECTOR_SIZE, fZ - _fWaterZOffset);
	RwIm3DVertexSetU    (&TempBufferRenderVertices[vidx + 2], TEXTURE_ADDU + 1.0f);
	RwIm3DVertexSetV    (&TempBufferRenderVertices[vidx + 2], TEXTURE_ADDV + 1.0f);
	RwIm3DVertexSetRGBA(&TempBufferRenderVertices[vidx + 2], color.red, color.green, color.blue,
		(uint8)(colorTrans.alpha + (color.alpha - colorTrans.alpha) * fAlphaBlendMulC));
	
	RwIm3DVertexSetPos  (&TempBufferRenderVertices[vidx + 3], fX + SMALL_SECTOR_SIZE, fY, fZ - _fWaterZOffset);
	RwIm3DVertexSetU    (&TempBufferRenderVertices[vidx + 3], TEXTURE_ADDU + 1.0f);
	RwIm3DVertexSetV    (&TempBufferRenderVertices[vidx + 3], TEXTURE_ADDV);
	RwIm3DVertexSetRGBA(&TempBufferRenderVertices[vidx + 3], color.red, color.green, color.blue,
		(uint8)(colorTrans.alpha + (color.alpha - colorTrans.alpha) * fAlphaBlendMulD));
