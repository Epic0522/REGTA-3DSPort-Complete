...
// Keep enough pool storage for compatibility, but cap live particles on 3DS.
...
#ifdef RW_3DS
// Opacity partners for the five rubber-smoke colour frames.
const char RubberMaskFiles[][8+1] =
{
	"rubber1m",
	"rubber2m",
	"rubber3m",
...
#ifdef RW_3DS
const char CardebrisMaskFiles[][13+1] =
{
	"cardebris_01m",
	"cardebris_02m",
	"cardebris_03m",
...
#ifdef RW_3DS
// PC particle TXDs often store colour and opacity as two dictionary textures
// (name + namem). librw returns the already-loaded colour texture immediately
// and therefore never applies the in-dictionary mask. Merge the pair once at
// particle initialisation; Raster::createFromImage then creates an ETC1A4
// texture, the same colour+alpha path used by the working flame sprites.
static RwTexture *
ReadMaskedParticleTexture(const char *name, const char *mask)
{
	RwTexture *colourTex = RwTextureRead(name, nil);
	if ( colourTex == nil || mask == nil )
		return colourTex;

	RwTexture *maskTex = RwTextureRead(mask, nil);
	if ( maskTex == nil )
		return colourTex;

	RwImage *colourImage = RwTextureGetRaster(colourTex)->toImage();
	RwImage *maskImage = RwTextureGetRaster(maskTex)->toImage();
	if ( colourImage == nil || maskImage == nil
		|| colourImage->width != maskImage->width
		|| colourImage->height != maskImage->height )
	{
		if ( colourImage ) colourImage->destroy();
		if ( maskImage ) maskImage->destroy();
		RwTextureDestroy(maskTex);
		return colourTex;
	}

	maskImage->makeMask();
	colourImage->applyMask(maskImage);
	RwRaster *mergedRaster = rw::Raster::createFromImage(colourImage);
	...
	if ( mergedRaster )
	{
		// Replace the dictionary texture's opaque raster rather than retaining a
		// second colour copy. The separate mask remains dictionary-owned, while
		// the active particle texture now has fire-style integrated alpha.
		RwRaster *opaqueRaster = RwTextureGetRaster(colourTex);
		RwTextureSetRaster(colourTex, mergedRaster);
		RwRasterDestroy(opaqueRaster);
	}
	return colourTex;
}
#endif
...
void CParticle::Initialise()
{
...
#ifdef RW_3DS
	gpSmoke2Tex = ReadMaskedParticleTexture("smokeII_3", "smokeII_3m");
#else
	...
	{
#ifdef RW_3DS
		gpRubberTex[i] = ReadMaskedParticleTexture(RubberFiles[i], RubberMaskFiles[i]);
#else
	...
	{
#ifdef RW_3DS
		gpCarDebrisTex[i] = ReadMaskedParticleTexture(CardebrisFiles[i], CardebrisMaskFiles[i]);
#else
...
#ifdef RW_3DS
	gpLeafTex = ReadMaskedParticleTexture("gameleaf01_64", "gameleaf01_64m");
#else
...
#ifdef RW_3DS
	gpLetterTex = ReadMaskedParticleTexture("letter", "letterm");
#else
...
#ifdef RW_3DS
	gpCloudTex4 = ReadMaskedParticleTexture("cloudmasked", "cloudmaskedm");
#else
...
#ifdef RW_3DS
	gpBloodSmallTex = ReadMaskedParticleTexture("bloodsplat2", "bloodsplat2m");
#else
...
#ifdef RW_3DS
	gpGungeTex = ReadMaskedParticleTexture("gunge", "gungem");
#else
...
#ifdef RW_3DS
	gpCollisionSmokeTex = ReadMaskedParticleTexture("collisionsmoke", "collisionsmokem");
#else
...
#ifdef RW_3DS
	gpBulletHitTex = ReadMaskedParticleTexture("bullethitsmoke", "bullethitsmokem");
#else
...
#ifdef RW_3DS
	gpGunShellTex = ReadMaskedParticleTexture("gunshell", "gunshellm");
#else
...
#ifdef RW_3DS
	gpHeatHazeTex = ReadMaskedParticleTexture("heathaze", "heathaze_m");
#else
...
#ifdef RW_3DS
	gpBeastieTex = ReadMaskedParticleTexture("beastie", "beastie_m");
#else
...
#ifdef RW_3DS
	gpRainDripTex[0] = ReadMaskedParticleTexture("raindrip64", "raindrip64_m");
#else
...
#ifdef RW_3DS
	gpRainDripTex[1] = ReadMaskedParticleTexture("raindripb64", "raindripb64_m");
#else
...
#ifdef _3DS
	/* VC constructs a muzzle flash out of as many as fifteen separate sprites.
	 * Budget only the high-frequency gun effects per frame; explosions, flames,
	 * weather and vehicle particles keep their existing independent behaviour. */
	static uint32 gunBudgetFrame = UINT32_MAX;
	static uint8 gunFlashCount;
	static uint8 gunSmokeCount;
	static uint8 gunShellCount;
	const uint32 currentFrame = CTimer::GetFrameCounter();
	if ( gunBudgetFrame != currentFrame )
	{
		gunBudgetFrame = currentFrame;
		gunFlashCount = 0;
		gunSmokeCount = 0;
		gunShellCount = 0;
	}

	if ( type == PARTICLE_GUNFLASH || type == PARTICLE_GUNFLASH_NOANIM )
	{
		if ( gunFlashCount >= 3 ) return nil;
		gunFlashCount++;
	}
	else if ( type == PARTICLE_GUNSMOKE || type == PARTICLE_GUNSMOKE2 )
	{
		if ( gunSmokeCount >= 2 ) return nil;
		gunSmokeCount++;
	}
	else if ( type >= PARTICLE_GUNSHELL_FIRST && type <= PARTICLE_GUNSHELL_BUMP2 )
	{
		if ( gunShellCount >= 2 ) return nil;
		gunShellCount++;
	}
#endif
