...
#ifdef _3DS
		// Multi-point contacts can submit the same pile-up many times in one
		// frame.  Preserve every player collision and budget ambient reports.
		static uint32 collisionBudgetFrame = UINT32_MAX;
		static uint8 ambientCollisionReports;
		const uint32 currentFrame = CTimer::GetFrameCounter();
		if(collisionBudgetFrame != currentFrame) {
			collisionBudgetFrame = currentFrame;
			ambientCollisionReports = 0;
		}
		const bool playerCollision =
			entity1->GetStatus() == STATUS_PLAYER || entity1->GetStatus() == STATUS_PLAYER_DISABLED ||
			entity2->GetStatus() == STATUS_PLAYER || entity2->GetStatus() == STATUS_PLAYER_DISABLED;
		if(!playerCollision) {
			if(ambientCollisionReports >= 6)
				return;
			ambientCollisionReports++;
		}
#endif
