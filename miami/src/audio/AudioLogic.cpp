...
#ifdef _3DS
		/* Decode the direct-NDSP line on core 2 while the script waits for the
		 * normal HAS_MISSION_AUDIO_LOADED result. */
		m_sMissionAudio.m_nLoadingStatus[slot] =
			SampleManager.PreparePreloadedStreamedFile(slot + 1) == 0 ?
			LOADING_STATUS_PREPARING : LOADING_STATUS_LOADED;
#else
		m_sMissionAudio.m_nLoadingStatus[slot] = LOADING_STATUS_LOADED;
#endif
		nFramesUntilFailedLoad[slot] = 0;
		break;
	case LOADING_STATUS_PREPARING:
#ifdef _3DS
		/* Failure only disables the optimization; Start() retains the synchronous
		 * fallback so mission audio can never be lost. */
		if(SampleManager.GetPreloadedStreamedFileStatus(slot + 1) != 0)
			m_sMissionAudio.m_nLoadingStatus[slot] = LOADING_STATUS_LOADED;
#endif
		break;
