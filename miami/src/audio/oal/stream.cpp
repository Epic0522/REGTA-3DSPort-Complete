...
protected:
	...
	CMP3File() :
		...
		m_nChannels(0) {}
public:
	CMP3File(const char *path, bool forceMono) :
		m_pMH(nil),
		m_bOpened(false),
		m_nRate(0),
		m_nChannels(0)
	{
		...
		if ( m_pMH )
		{
			...
			long decoderFlags = MPG123_FUZZY | MPG123_SEEKBUFFER | MPG123_GAPLESS | MPG123_QUIET;
#ifdef _3DS
			/* The 3DS output mix is mono, so do not decode an unused stereo
			 * channel for radio and streamed music. */
			if(forceMono)
				decoderFlags |= MPG123_MONO_MIX;
#else
			(void)forceMono;
#endif
			mpg123_param(m_pMH, MPG123_FLAGS, decoderFlags, 0.0);
#endif
			long rate = 0;
			int channels = 0;
			int encoding = 0;
	...
	uint32 Decode(void *buffer)
	{
		...
		size_t size = 0;
		int err = MPG123_ERR;
		/* mpg123 may report a format transition before yielding samples.  The
		 * stream format is pinned at open time, so consume that notification once
		 * instead of treating it as a dead track and reopening it every few frames. */
		for(int attempt = 0; attempt < 2; attempt++){
			err = mpg123_read(m_pMH, (unsigned char *)buffer, GetBufferSize(), &size);
...
// Be case-insensitive on linux (from https://github.com/OneSadCookie/fcaseopen/)
	...
	if (!strcasecmp(&m_aFilename[strlen(m_aFilename) - strlen(".wav")], ".wav"))
		...
		m_pSoundFile = new CMP3File(m_aFilename, forceMonoDecode);
	else if (!strcasecmp(&m_aFilename[strlen(m_aFilename) - strlen(".adf")], ".adf"))
		m_pSoundFile = new CADFFile(m_aFilename, forceMonoDecode);
#endif
...
bool CStream::UsesDirectNDSP() const
{
	return m_nDirectChannel != 0;
}

void CStream::ApplyDirectNDSPMix()
{
	if(!UsesDirectNDSP())
		return;
	const float gain = (float)m_nVolume / MAX_VOLUME;
	const float pan = (float)clamp((int32)m_nPan, 0, 127) / 127.0f;
	float mix[12] = { 0.0f };
	mix[0] = gain * (pan <= 0.5f ? 1.0f : (1.0f - pan) * 2.0f);
	mix[1] = gain * (pan >= 0.5f ? 1.0f : pan * 2.0f);
	ndspChnSetMix(m_nDirectChannel, mix);
}

void CStream::ReleaseDirectNDSPBuffer()
{
	if(!UsesDirectNDSP())
		return;
	ndspChnWaveBufClear(m_nDirectChannel);
	if(m_pDirectBuffer != nil){
		linearFree(m_pDirectBuffer);
		m_pDirectBuffer = nil;
	}
	m_nDirectBytes = 0;
	memset(&m_DirectWaveBuf, 0, sizeof(m_DirectWaveBuf));
	if(__atomic_load_n(&m_nPrepareState, __ATOMIC_ACQUIRE) != PREPARE_RUNNING)
		__atomic_store_n(&m_nPrepareState, PREPARE_IDLE, __ATOMIC_RELEASE);
}

bool CStream::DecodeDirectNDSPBuffer()
{
	if(!m_bFullInitialBuffer || !UsesDirectNDSP() || !IsOpened())
		return false;
	const uint32 channels = m_pSoundFile->GetChannels();
	const uint32 sampleSize = m_pSoundFile->GetSampleSize();
	const uint64 totalBytes64 = (uint64)m_pSoundFile->GetSampleCount() * channels * sampleSize;
	if(channels != 1 || totalBytes64 == 0 || totalBytes64 > 1024*1024)
		return false;

	uint8 *whole = (uint8*)linearAlloc((size_t)totalBytes64);
	if(whole == nil)
		return false;
	const uint32 startMs = m_pSoundFile->Tell();
	uint32 written = 0;
	while(written < (uint32)totalBytes64){
		uint32 decoded = m_pSoundFile->Decode(m_pBuffer);
		if(decoded == 0)
			break;
		decoded = Min(decoded, (uint32)totalBytes64 - written);
		memcpy(whole + written, m_pBuffer, decoded);
		written += decoded;
	}
	if(written == 0){
		linearFree(whole);
	...
	m_pDirectBuffer = whole;
	m_nDirectBytes = written;
	m_nWholeBufferStartMs = startMs;
	return true;
}

void CStream::PrepareThreadMain(void *arg)
{
	CStream *stream = (CStream*)arg;
	bool ok = stream->DecodeDirectNDSPBuffer();
	__atomic_store_n(&stream->m_nPrepareState,
		ok ? PREPARE_READY : PREPARE_FAILED, __ATOMIC_RELEASE);
}

void CStream::JoinPrepareThread()
{
	if(m_PrepareThread != nil){
		threadJoin(m_PrepareThread, U64_MAX);
		threadFree(m_PrepareThread);
		m_PrepareThread = nil;
	}
}

int8 CStream::BeginPrepare()
{
	if(!m_bFullInitialBuffer || !UsesDirectNDSP() || !IsOpened())
		return 1;
	int32 state = __atomic_load_n(&m_nPrepareState, __ATOMIC_ACQUIRE);
	if(state == PREPARE_READY)
		return 1;
	if(state == PREPARE_RUNNING)
		return 0;

	ReleaseDirectNDSPBuffer();
	__atomic_store_n(&m_nPrepareState, PREPARE_RUNNING, __ATOMIC_RELEASE);
	/* Decode complete phone/mission lines away from the gameplay core.  A lower
	 * priority than the existing core-2 streaming worker keeps map IO first. */
	m_PrepareThread = threadCreate(PrepareThreadMain, this, 128*1024, 0x31, 2, false);
	if(m_PrepareThread == nil){
		__atomic_store_n(&m_nPrepareState, PREPARE_IDLE, __ATOMIC_RELEASE);
		return -1;
	}
...
int8 CStream::GetPrepareStatus() const
{
	int32 state = __atomic_load_n(&m_nPrepareState, __ATOMIC_ACQUIRE);
	if(state == PREPARE_READY)
		return 1;
...
{
	return m_pAlSources[0] != AL_NONE &&
		(m_bSingleSource || m_pAlSources[1] != AL_NONE);
}
...
#ifdef _3DS
	if(UsesDirectNDSP())
		return !m_bPaused && ndspChnIsPlaying(m_nDirectChannel);
#endif
...
#ifdef _3DS
	if(UsesDirectNDSP()){
		ndspChnSetPaused(m_nDirectChannel, true);
		return;
	}
#endif
...
void CStream::SetPitch(float pitch)
{
	...
	if(UsesDirectNDSP()){
		if(IsOpened())
			ndspChnSetRate(m_nDirectChannel, m_pSoundFile->GetSampleRate() * pitch);
		return;
	}
#endif
...
#endif
	if(m_bSingleSource){
		const float x = (clamp((int32)nPan, 0, 127) - 63) / 64.0f;
		SetPosition(0, x, 0.0f, Sqrt(fmax(0.0f, 1.0f - SQR(x))));
		return;
	}
...
#ifdef _3DS
	if(UsesDirectNDSP())
		return m_nWholeBufferStartMs +
			m_pSoundFile->samples2ms(ndspChnGetSamplePos(m_nDirectChannel));
#endif
	
	ALint offset;
	//alGetSourcei(m_alSource, AL_SAMPLE_OFFSET, &offset);
	alGetSourcei(m_pAlSources[0], AL_BYTE_OFFSET, &offset);
	if(m_bWholeFileBuffer)
		return m_nWholeBufferStartMs +
...
bool CStream::FillBuffer(ALuint *alBuffer)
{
	...
	if ( !(alBuffer[0] != AL_NONE && alIsBuffer(alBuffer[0])) )
		return false;
	if ( !m_bSingleSource && !(alBuffer[1] != AL_NONE && alIsBuffer(alBuffer[1])) )
		return false;
	
	uint32 size = m_pSoundFile->Decode(m_pBuffer);
	if (size == 0 && m_bSeamlessLoop && m_nLoopCount != 1) {
		/* Refill across the loop point before the OpenAL queue can drain.  The
		 * old empty-queue recovery only queued one block on 3DS and permanently
		 * removed the stream's underrun headroom after its first pass. */
		m_pSoundFile->Seek(0);
		if (m_nLoopCount > 1)
			m_nLoopCount--;
		size = m_pSoundFile->Decode(m_pBuffer);
	}
	if( size == 0 )
		return false;
...
int32 CStream::FillWholeFileBuffer()
{
#ifndef _3DS
	return 0;
#else
	if(!m_bFullInitialBuffer || !IsOpened() || !HasSource())
		return 0;

	const uint32 channels = m_pSoundFile->GetChannels();
	const uint32 sampleSize = m_pSoundFile->GetSampleSize();
	const uint64 totalBytes64 = (uint64)m_pSoundFile->GetSampleCount() * channels * sampleSize;
	/* The longest shipped VC dialogue line is under 0.4 MiB decoded.  Keep a
	 * generous ceiling, but never turn a radio/music stream into a large PCM
	 * allocation if it reaches a mission stream slot. */
	if(channels == 0 || channels > 2 || totalBytes64 == 0 || totalBytes64 > 1024*1024)
		return 0;

	const uint32 bytesPerChannel = (uint32)totalBytes64 / channels;
	const bool directNDSP = UsesDirectNDSP() && channels == 1;
	if(directNDSP){
		int32 state = __atomic_load_n(&m_nPrepareState, __ATOMIC_ACQUIRE);
		if(state == PREPARE_RUNNING){
			JoinPrepareThread();
			state = __atomic_load_n(&m_nPrepareState, __ATOMIC_ACQUIRE);
		}else if(state == PREPARE_READY || state == PREPARE_FAILED)
			JoinPrepareThread();
		if(state != PREPARE_READY){
			ReleaseDirectNDSPBuffer();
			if(!DecodeDirectNDSPBuffer())
				return 0;
			__atomic_store_n(&m_nPrepareState, PREPARE_READY, __ATOMIC_RELEASE);
		}

		memset(&m_DirectWaveBuf, 0, sizeof(m_DirectWaveBuf));
		m_DirectWaveBuf.data_pcm16 = (int16*)m_pDirectBuffer;
		m_DirectWaveBuf.nsamples = m_nDirectBytes / sizeof(int16);
		m_DirectWaveBuf.looping = false;
		ndspChnReset(m_nDirectChannel);
		ndspChnSetInterp(m_nDirectChannel, NDSP_INTERP_LINEAR);
		ndspChnSetFormat(m_nDirectChannel, NDSP_FORMAT_MONO_PCM16);
		ndspChnSetRate(m_nDirectChannel, m_pSoundFile->GetSampleRate());
		ApplyDirectNDSPMix();
		DSP_FlushDataCache(m_pDirectBuffer, m_nDirectBytes);
		ndspChnWaveBufAdd(m_nDirectChannel, &m_DirectWaveBuf);
		m_bWholeFileBuffer = true;
		return 1;
	}

	uint8 *whole = (uint8*)malloc((size_t)totalBytes64);
	if(whole == nil)
		return 0;

	m_nWholeBufferStartMs = m_pSoundFile->Tell();
	uint32 writtenPerChannel = 0;
	while(writtenPerChannel < bytesPerChannel){
		const uint32 decoded = m_pSoundFile->Decode(m_pBuffer);
		if(decoded == 0)
			break;
		uint32 decodedPerChannel = decoded / channels;
		if(decodedPerChannel > bytesPerChannel - writtenPerChannel)
			decodedPerChannel = bytesPerChannel - writtenPerChannel;
		for(uint32 channel = 0; channel < channels; channel++)
			memcpy(whole + channel*bytesPerChannel + writtenPerChannel,
				(uint8*)m_pBuffer + channel*(decoded/channels), decodedPerChannel);
		writtenPerChannel += decodedPerChannel;
	}

	if(writtenPerChannel == 0){
		free(whole);
		return 0;
	}

	alBufferData(m_alBuffers[0], AL_FORMAT_MONO16, whole,
		writtenPerChannel, m_pSoundFile->GetSampleRate());
	if(channels == 1)
		alBufferData(m_alBuffers[1], AL_FORMAT_MONO16, whole,
			writtenPerChannel, m_pSoundFile->GetSampleRate());
	else
		alBufferData(m_alBuffers[1], AL_FORMAT_MONO16, whole + bytesPerChannel,
			writtenPerChannel, m_pSoundFile->GetSampleRate());
	alSourceQueueBuffers(m_pAlSources[0], 1, &m_alBuffers[0]);
	if(!m_bSingleSource)
		alSourceQueueBuffers(m_pAlSources[1], 1, &m_alBuffers[1]);
	free(whole);
	m_bWholeFileBuffer = true;
	return 1;
#endif
...
int32 CStream::FillBuffers()
{
	int32 i = 0;
#ifdef _3DS
	if(m_bFullInitialBuffer){
		const int32 wholeFileBuffers = FillWholeFileBuffer();
		if(wholeFileBuffers != 0)
			return wholeFileBuffers;
	}
#endif
#ifdef _3DS
	/* Keep the low-latency half-second startup for radio stream 0, but give
	 * preloaded mission dialogue the original full second.  Long cutscene frames
	 * can otherwise starve the 3DS OpenAL queue; restarting a starved source may
	 * replay the tail of the previous chunk as a repeated syllable. */
	const int32 initialBufferPairs = (m_bFullInitialBuffer || m_bFullInitialQueue) ?
		NUM_STREAMBUFFERS/2 : 2;
#else
	const int32 initialBufferPairs = NUM_STREAMBUFFERS/2;
#endif
	for ( i = 0; i < initialBufferPairs; i++ )
	{
...
void CStream::SetPlay(bool state)
{
	...
	if(UsesDirectNDSP()){
		if(state)
			ndspChnSetPaused(m_nDirectChannel, false);
		else
			ndspChnWaveBufClear(m_nDirectChannel);
		m_bActive = state;
		return;
...
void CStream::Update()
{
	...
	if ( !m_bPaused )
	{
		...
		/* Preserve the original paired-source path for mission/cutscene WAVs.
		 * The bounded/asymmetric variants tested on hardware could restart one
		 * source at a chunk boundary and repeat part of a word.  Radio keeps the
		 * non-blocking minimum below because its decoder is the path that can fail
		 * and must never trap the game in the original equality loop. */
		ALint pairedBuffersProcessed;
		if(m_bSingleSource){
			pairedBuffersProcessed = buffersProcessed[0];
		}else if(m_bFullInitialBuffer){
			do {
				alGetSourcei(m_pAlSources[0], AL_BUFFERS_PROCESSED, &buffersProcessed[0]);
				alGetSourcei(m_pAlSources[1], AL_BUFFERS_PROCESSED, &buffersProcessed[1]);
			...
			pairedBuffersProcessed = Min(buffersProcessed[0], buffersProcessed[1]);

#ifdef _3DS
		/* A long stream may request the full initial queue while being too large
		 * for whole-file buffering.  Keep its per-frame refill bounded just like
		 * radio; only genuinely whole-file-buffered dialogue can skip the limit. */
		const bool limitDecodeWork = !m_bWholeFileBuffer;
		/* Stream 0 is radio/music.  A script-heavy frame may leave several of its
		 * 250 ms chunks processed at once.  Decoding the entire backlog in the next
		 * frame creates a feedback loop: the refill itself makes another long frame,
		 * more chunks expire, and gameplay can remain extremely slow.  One chunk per
		 * frame is far above the steady-state rate while bounding the catch-up cost. */
		if(limitDecodeWork){
			const ALint maxRefills = m_bMissionMusic ? 2 : 1;
			if(pairedBuffersProcessed > maxRefills)
				pairedBuffersProcessed = maxRefills;
		}
#else
		const bool limitDecodeWork = false;
#endif
		...
		if (m_nLoopCount != 1 && m_bActive && totalBuffers[0] == 0)
		{
			Setup(true);
#ifdef _3DS
			if(limitDecodeWork && !m_bMissionMusic){
				/* Recover an empty radio queue without doing a multi-chunk refill in
				 * the same frame that already missed its audio deadline. */
				if(FillBuffer(&m_alBuffers[0])){
					alSourceQueueBuffers(m_pAlSources[0], 1, &m_alBuffers[0]);
					if(!m_bSingleSource)
						alSourceQueueBuffers(m_pAlSources[1], 1, &m_alBuffers[1]);
