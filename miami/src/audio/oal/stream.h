#pragma once

#ifdef AUDIO_OAL
#include <AL/al.h>
#ifdef _3DS
#include <3ds.h>
#endif

#define NUM_STREAMBUFFERS 8

class IDecoder
{
public:
	virtual ~IDecoder() { }
	
	virtual bool   IsOpened() = 0;
	
	virtual uint32 GetSampleSize() = 0;
	virtual uint32 GetSampleCount() = 0;
	virtual uint32 GetSampleRate() = 0;
	virtual uint32 GetChannels() = 0;
	
	uint32 GetAvgSamplesPerSec()
	{
		return GetChannels() * GetSampleRate();
	}
	
	uint32 ms2samples(uint32 ms)
	{
		return float(ms) / 1000.0f * float(GetSampleRate());
	}
	
	uint32 samples2ms(uint32 sm)
	{
		return float(sm) * 1000.0f / float(GetSampleRate());
	}
	
	uint32 GetBufferSamples()
	{
		//return (GetAvgSamplesPerSec() >> 2) - (GetSampleCount() % GetChannels());
		return (GetAvgSamplesPerSec() / 4); // 250ms
	}
	
	uint32 GetBufferSize()
	{
		return GetBufferSamples() * GetSampleSize();
	}
	
	virtual void   Seek(uint32 milliseconds) = 0;
	virtual uint32 Tell() = 0;
	
	uint32 GetLength()
	{
		return float(GetSampleCount()) * 1000.0f / float(GetSampleRate());
	}
	
	virtual uint32 Decode(void *buffer) = 0;
};

class CStream
{
	char     m_aFilename[128];
	ALuint  *m_pAlSources;
	ALuint (&m_alBuffers)[NUM_STREAMBUFFERS];
	
	bool     m_bPaused;
	bool     m_bActive;
	
	void    *m_pBuffer;
	
	bool     m_bReset;
	uint32   m_nVolume;
	uint8    m_nPan;
	uint32   m_nPosBeforeReset;
	int32   m_nLoopCount;
	bool     m_bSeamlessLoop;
	bool     m_bMissionMusic;
	bool    m_bFullInitialBuffer;
	bool    m_bFullInitialQueue;
	bool    m_bSingleSource;
	bool    m_bWholeFileBuffer;
	uint32  m_nWholeBufferStartMs;
#ifdef _3DS
	uint8   m_nDirectChannel;
	uint8  *m_pDirectBuffer;
	uint32  m_nDirectBytes;
	ndspWaveBuf m_DirectWaveBuf;
	Thread  m_PrepareThread;
	volatile int32 m_nPrepareState;
	Thread  m_RadioStartThread;
	volatile int32 m_nRadioStartState;
#endif
	
	IDecoder *m_pSoundFile;
	
	bool HasSource();
	void SetPosition(int i, float x, float y, float z);
	void SetPitch(float pitch);
	void SetGain(float gain);
	void   Pause();
	void   SetPlay(bool state);
	
	bool   FillBuffer(ALuint *alBuffer);
	int32  FillWholeFileBuffer();
	int32  FillBuffers();
	void   ClearBuffers();
#ifdef _3DS
	bool   UsesDirectNDSP() const;
	void   ApplyDirectNDSPMix();
	void   ReleaseDirectNDSPBuffer();
	bool   DecodeDirectNDSPBuffer();
	void   JoinPrepareThread();
	static void PrepareThreadMain(void *arg);
	void   JoinRadioStartThread();
	static void RadioStartThreadMain(void *arg);
#endif
public:
	static void Initialise();
	static void Terminate();
	
	CStream(ALuint *sources, ALuint (&buffers)[NUM_STREAMBUFFERS]);
	CStream(char *filename, ALuint *sources, ALuint (&buffers)[NUM_STREAMBUFFERS], uint32 overrideSampleRate = 32000,
		bool fullInitialBuffer = false, bool forceMonoDecode = true, uint8 directChannel = 0);
	~CStream();
	void   Delete();
	bool   Open(const char *filename, uint32 overrideSampleRate = 32000,
		bool fullInitialBuffer = false, bool forceMonoDecode = true, uint8 directChannel = 0);
	void   Close();
	
	bool   IsOpened();
	bool   IsPlaying();
	void   SetPause (bool bPause);
	void   SetVolume(uint32 nVol);
	void   SetPan   (uint8 nPan);
	void   SetPosMS (uint32 nPos); 
	uint32 GetPosMS();
	uint32 GetLengthMS();
	bool   HasDecodedToEnd();
	
	bool Setup(bool imSureQueueIsEmpty = false);
	void Start();
	void Stop();
	void Update(void);
	void SetLoopCount(int32);
	void SetSeamlessLoop(bool enabled) { m_bSeamlessLoop = enabled; }
	void SetMissionMusic(bool enabled) { m_bMissionMusic = enabled; }
	void SetFullInitialQueue(bool enabled) { m_bFullInitialQueue = enabled; }
#ifdef _3DS
	int8 BeginPrepare();
	int8 GetPrepareStatus() const;
	bool BeginRadioStart();
#endif

	
	void ProviderInit();
	void ProviderTerm();
};

#endif
