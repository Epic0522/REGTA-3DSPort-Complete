...
{
#ifdef ENABLE_3DS_LOADING_PROGRESS
	SetLoadingScreenStage(0.60f, "Reading animation archive");
#endif
	LoadAnimFile("ANIM\\PED.IFP");
#ifdef ENABLE_3DS_LOADING_PROGRESS
	SetLoadingScreenStage(0.65f, "Building animation groups");
#endif
...
CAnimManager::CreateAnimAssocGroups(void)
{
	...
	for(i = 0; i < NUM_ANIM_ASSOC_GROUPS; i++){
#ifdef ENABLE_3DS_LOADING_PROGRESS
		SetLoadingScreenProgress(0.65f + 0.02f *
			(float)i/(float)NUM_ANIM_ASSOC_GROUPS);
#endif
...
CAnimManager::LoadAnimFile(const char *filename)
{
...
#ifdef ENABLE_3DS_ANIM_PRELOAD
	// PED.IFP is only a couple of megabytes, but parsing it through StreamFile
	// used to issue a separate fread for every 20-44 byte keyframe. Thousands
	// of tiny FS calls dominate startup time on a 3DS SD card. Read the archive
	// once and retain only the parsed animation structures afterwards.
	stream->seek(0, 2);
	...
	if(fileData){
		...
		if(bytesRead == fileSize){
			...
			assert(memoryStream);
#ifdef ENABLE_3DS_LOADING_PROGRESS
			SetLoadingScreenStage(0.61f, "Parsing animations");
#endif
...
#ifdef ENABLE_3DS_LOADING_PROGRESS
	SetLoadingScreenStage(0.60f, "Animation preload fallback");
#endif
#endif
...
#ifdef ENABLE_3DS_LOADING_PROGRESS
		SetLoadingScreenProgress(0.61f + 0.04f *
			(float)j/(float)animBlock->numAnims);
#endif
