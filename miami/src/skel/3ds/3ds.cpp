...
stateMachine()
{
	...
	case GS_INIT_ONCE:
		...
		#ifndef EMULATOR_SKIP_STARTUP_MOVIES
			Play3DSStartupMovies();
		#endif
		}
...
ctrInit()
{
	...
	/* Existing reVC installations, including every previously verified 3DS
	 * release, keep their data and saves in /3ds/miami.  The unified-tree
	 * rename accidentally changed this to /3ds/revc, so a valid old install
	 * exited before RenderWare was initialised.  Keep the newer spelling as a
	 * compatibility fallback, but preserve miami as the primary contract. */
	if(chdir("sdmc:/3ds/miami") != 0 && chdir("sdmc:/3ds/revc") != 0){
		printf("Neither sdmc:/3ds/miami nor sdmc:/3ds/revc is present.\n");
		/* exit() runs C++ global destructors.  Several global collision models
		 * call RwFree there even though RenderWare has not started yet, masking
		 * this simple missing-directory error as a PC=0 prefetch abort. */
		svcExitProcess();
	}
}
