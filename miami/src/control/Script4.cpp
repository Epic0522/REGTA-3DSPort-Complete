...
int8 CRunningScript::ProcessCommands900To999(int32 command)
{
	...
	*/
	...
	case COMMAND_LOAD_MISSION_AUDIO:
	{
		...
		FinalMissionMusic::ObserveVCMissionAudioLoaded(str, ScriptParams[0] - 1);
		return 0;
	}
	...
	case COMMAND_PLAY_MISSION_AUDIO:
		...
		FinalMissionMusic::ObserveVCMissionAudioPlayed(ScriptParams[0] - 1);
		return 0;
	case COMMAND_HAS_MISSION_AUDIO_FINISHED:
	{
		CollectParameters(&m_nIp, 1);
		const uint8 slot = ScriptParams[0] - 1;
		const bool finished = DMAudio.IsMissionAudioSampleFinished(slot);
		UpdateCompareFlag(finished);
		if (finished)
			FinalMissionMusic::ObserveVCMissionAudioFinished(slot);
		return 0;
	}
