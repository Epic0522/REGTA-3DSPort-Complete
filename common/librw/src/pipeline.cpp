...
static inline int8
packCompNormal(float component, float scale)
{
	float packed = component * scale;
	if(!isfinite(packed))
		return 0;
	if(packed > 127.0f)
		packed = 127.0f;
	else if(packed < -127.0f)
		packed = -127.0f;
	return (int8)packed;
}
...
instV3d(int type, uint8 *dst, V3d *src, uint32 numVertices, uint32 stride)
{
	if(type == VERT_FLOAT3)
		...
		for(uint32 i = 0; i < numVertices; i++){
			...
			if(!isfinite(m) || m <= 0.000001f){
				((int8*)dst)[0] = 0;
				((int8*)dst)[1] = 0;
				((int8*)dst)[2] = 127;
			}else{
				float s = 127.0f / m;
				((int8*)dst)[0] = packCompNormal(src->x, s);
				((int8*)dst)[1] = packCompNormal(src->y, s);
				((int8*)dst)[2] = packCompNormal(src->z, s);
			}
			((int8*)dst)[3] = 0;
			dst += stride;
			src++;
