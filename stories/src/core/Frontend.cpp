...
#ifdef _3DS
// The LCS atlas is 320x220.  A 224-unit menu half-size becomes 120 physical
// pixels, filling the 240-pixel top screen without stretching it into a square.
#define MAP_MIN_SIZE 224.f
#else
...
static wchar *
GetPlatformFrontendText(const char *key)
{
	/* LCS' original save strings describe PS2 memory-card hardware, and some
	...
	static t3DSSaveText saveText[] = {
		{ "3DS_MPH", "C-PAD: MOVE  ZR/R: ZOOM  L: LEGEND  X: MARKER  B: BACK", {}, false },
		...
		{ "FES_SSC", "Game saved successfully.", {}, false },
		{ "FESZ_WR", "Saving game data. Do not turn off the system or remove the SD card.", {}, false },
		{ "FES_CMP", "Unable to save game data. Check the SD card and try again.", {}, false },
		{ "FEC_SVU", "Unable to save game data. Check the SD card and try again.", {}, false },
	};

	for (uint32 i = 0; i < ARRAY_SIZE(saveText); i++) {
		if (strcmp(key, saveText[i].key) == 0) {
			if (!saveText[i].ready) {
				AsciiToUnicode(saveText[i].ascii, saveText[i].unicode);
				saveText[i].ready = true;
			}
			return saveText[i].unicode;
		}
	}
...
CMenuManager::CMenuManager()
{
	...
	m_fMapSize = MENU_Y(MAP_MIN_SIZE); // Y because of HOR+
...
#ifdef GAMEPAD_MENU
#if defined(__SWITCH__) || defined(_3DS)
...
{
#ifdef _3DS
	const bool instantPause = !m_bGameNotLoaded && !m_OnlySaveMenu;
	if (!instantPause) {
#endif
	DoRWStuffStartOfFrame(0, 0, 0, 0, 0, 0, 255);
	DoRWStuffEndOfFrame();
	...
	#ifdef _3DS
	if (instantPause) {
		Set3DSGameStreamPausedForMenu(TRUE);
	} else
	#endif
	...
	//CFileMgr::SetDir("");
#ifdef _3DS
	// Save-marker entry still needs slot information, ordinary pause does not.
	if (m_OnlySaveMenu)
#endif
		PcSaveHelper.PopulateSlotInfo();
...
CMenuManager::DisplayHelperText(char *text)
{
	...
	if (text) {
		...
		CFont::PrintString(SCREEN_STRETCH_FROM_RIGHT(HELPER_TEXT_RIGHT_MARGIN), SCREEN_SCALE_FROM_BOTTOM(HELPER_TEXT_BOTTOM_MARGIN), GetPlatformFrontendText(text));
		return;
	}
...
CMenuManager::DrawStandardMenus(bool activeScreen)
{
	...
	switch (m_nCurrScreen) {
		...
		case MENUPAGE_CHOOSE_SAVE_SLOT:
			...
			break;*/
		case MENUPAGE_SOUND_SETTINGS:
			/* The original LCS PS2 archives do not provide a matching set of
			 * radio-logo textures for this reVC selector.  Keep the functional
			 * station row and its LCS text, but do not display Vice City logos. */
#ifndef _3DS
			PrintRadioSelector();
#endif
			break;
	...
	if (aScreens[m_nCurrScreen].m_ScreenName[0] != '\0') {
		...
		CFont::PrintString(SCREEN_STRETCH_X(MENUHEADER_POS_X), SCREEN_SCALE_Y(MENUHEADER_POS_Y), GetPlatformFrontendText(aScreens[m_nCurrScreen].m_ScreenName));
	}
	...
	if (aScreens[m_nCurrScreen].m_aEntries[0].m_Action == MENUACTION_LABEL) {
		...
		case MENUPAGE_SAVE_OVERWRITE_CONFIRM:
			if (Slots[m_nCurrSaveSlot] == SLOT_OK)
				str = GetPlatformFrontendText("FESZ_QO");
			else if (Slots[m_nCurrSaveSlot] == SLOT_CORRUPTED)
				str = GetPlatformFrontendText("FESZ_QZ");
			else
				str = GetPlatformFrontendText(aScreens[m_nCurrScreen].m_aEntries[0].m_EntryName);
			break;
		case MENUPAGE_EXIT:
			if (m_bGameNotLoaded)
		...
		for (int i = 0; i < NUM_MENUROWS; ++i) {
			...
			if (aScreens[m_nCurrScreen].m_aEntries[i].m_Action != MENUACTION_LABEL && aScreens[m_nCurrScreen].m_aEntries[i].m_EntryName[0] != '\0') {
				...
				if (aScreens[m_nCurrScreen].m_aEntries[i].m_SaveSlot >= SAVESLOT_1 && aScreens[m_nCurrScreen].m_aEntries[i].m_SaveSlot <= SAVESLOT_8) {
					...
					leftText = GetPlatformFrontendText(aScreens[m_nCurrScreen].m_aEntries[i].m_EntryName);
				}
...
CMenuManager::DrawBackground(bool transitionCall)
{
	if (!m_bSpritesLoaded) {
#ifdef _3DS
		if (!m_bGameNotLoaded && m_nCurrScreen == MENUPAGE_PAUSE_MENU && gPauseBackgroundResident3DS) {
			SetFrontEndRenderStates();
			m_aFrontEndSprites[MENUSPRITE_BACKGROUND].Draw(CRect(0.0f, 0.0f, SCREEN_WIDTH, SCREEN_HEIGHT), CRGBA(255, 255, 255, 255));
...
CMenuManager::LoadAllTextures()
{
...
#ifdef _3DS
	bool frontendTxdSlot1NeedsCache = LoadMenuTextureCache3DS(frontendTxdSlot1, "MODELS/FRONTEN1.TXD");
#else
	...
	for (int i = 0; i < 3; i++) {
#ifdef _3DS
		if (i == MENUSPRITE_BACKGROUND && gPauseBackgroundResident3DS)
			continue;
#endif
...
#ifdef _3DS
	FinishMenuTextureCache3DS(frontendTxdSlot1, "MODELS/FRONTEN1.TXD", frontendTxdSlot1NeedsCache);
#endif
...
#ifdef _3DS
		bool frontendTxdSlot2NeedsCache = LoadMenuTextureCache3DS(frontendTxdSlot2, "MODELS/FRONTEN2.TXD");
#else
...
#endif
#if defined(_3DS) && defined(ENABLE_3DS_BOTTOM_RADAR)
			/* fronten2 still contains Vice City's obsolete 3x3 map.  Never load
			 * those tiles on 3DS: Liberty City's embedded radar atlas is the
			 * sole menu-map source. */
			if(i >= MENUSPRITE_MAPTOP01 && i <= MENUSPRITE_MAPBOT03)
				continue;
#endif
#ifdef _3DS
			/* fronten2.txd contains Vice City radio logos.  LCS station names
			 * remain available through the normal textual MENUACTION_RADIO row. */
			if(i >= MENUSPRITE_WILDSTYLE && i <= MENUSPRITE_MP3)
				continue;
#endif
			m_aFrontEndSprites[i].SetTexture(FrontendFilenames[i][0], FrontendFilenames[i][1]);
			m_aFrontEndSprites[i].SetAddressing(rwTEXTUREADDRESSBORDER);
...
#ifdef _3DS
		FinishMenuTextureCache3DS(frontendTxdSlot2, "MODELS/FRONTEN2.TXD", frontendTxdSlot2NeedsCache);
#endif
		CTxdStore::PopCurrentTxd();
...
CMenuManager::SmallMessageScreen(const char* text)
{
	...
	wchar *message = GetPlatformFrontendText(text);
	int numOfLines = CFont::GetNumberLines(SCREEN_WIDTH / 2.f, SCREEN_SCALE_Y(135.f), message);
	float y;
	if (numOfLines > 1)
	...
	CFont::PrintString(SCREEN_WIDTH / 2.f, y, message);
}
...
#endif
					#ifdef _3DS
					const float mapHalfWidth = m_fMapSize * (320.0f / 220.0f);
					if (mapCrosshair.y > m_fMapCenterY - m_fMapSize && mapCrosshair.y < m_fMapCenterY + m_fMapSize &&
						mapCrosshair.x > m_fMapCenterX - mapHalfWidth && mapCrosshair.x < m_fMapCenterX + mapHalfWidth) {
						const float sourceScale = m_fMapSize * 2.0f / 220.0f;
						const float sourceX = (mapCrosshair.x -
							(m_fMapCenterX - mapHalfWidth)) / sourceScale;
						const float sourceY = (mapCrosshair.y -
							(m_fMapCenterY - m_fMapSize)) / sourceScale;
						const float atlasX = sourceX - LCS_MENU_MAP_X_OFFSET;
						const float atlasY = sourceY - LCS_MENU_MAP_Y_OFFSET;
						const float determinant = LCS_MENU_MAP_XX * LCS_MENU_MAP_YY -
							LCS_MENU_MAP_XY * LCS_MENU_MAP_YX;
						CRadar::ToggleTargetMarker(
							(atlasX * LCS_MENU_MAP_YY - atlasY * LCS_MENU_MAP_XY) / determinant,
							(LCS_MENU_MAP_XX * atlasY - LCS_MENU_MAP_YX * atlasX) / determinant);
...
#undef ZOOM
			...
			if (!pressedL) {
				if (CPad::GetPad(0)->GetChar('L') || CPad::GetPad(0)->GetChar('l') ||
					CPad::GetPad(0)->GetLeftShoulder1JustDown()) {
...
CMenuManager::SwitchMenuOnAndOff()
{
	if (!TheCamera.m_WideScreenOn) {
		...
		if ((CPad::GetPad(0)->GetStartJustDown() || CPad::GetPad(0)->GetEscapeJustDown())
			...
			const bool instantPauseEntry = !m_bMenuActive && !m_bGameNotLoaded &&
				!m_bShutDownFrontEndRequested && !m_bStartUpFrontEndRequested;

			if (!instantPauseEntry && m_nCurrScreen != MENUPAGE_LOADING_IN_PROGRESS
#ifdef XBOX_MESSAGE_SCREEN
			...
			if (m_bMenuActive) {
				...
				Initialise();
			#ifdef _3DS
				if (instantPauseEntry)
					gPauseInputDelayFrames3DS = 2;
			#endif
			#ifdef _3DS
				if (m_bGameNotLoaded || m_OnlySaveMenu)
			#endif
				LoadAllTextures();
#ifdef FIX_BUGS
				CPad::StopPadsShaking();
...
#ifdef _3DS
	if (!m_bGameNotLoaded && !m_OnlySaveMenu)
		Set3DSGameStreamPausedForMenu(FALSE);
	else
#endif
...
CMenuManager::ReleasePauseHomeTextures()
{
	...
	if (gPauseBackgroundResident3DS) {
		m_aFrontEndSprites[MENUSPRITE_BACKGROUND].Delete();
...
#if defined(_3DS) && defined(ENABLE_3DS_BOTTOM_RADAR)
	/* fronten2 is a retained VC compatibility dictionary in LCS.  Draw the
	 * radar-derived Liberty City atlas instead, with no VC fallback. */
	CSprite2d::DrawRect(CRect(0.0f, 0.0f, SCREEN_WIDTH, SCREEN_HEIGHT),
		CRGBA(0, 0, 0, 255));
	const float lcsMapHalfWidth = m_fMapSize * (320.0f / 220.0f);
	DrawLCSMenuMap3DS(CRect(
		m_fMapCenterX - lcsMapHalfWidth, m_fMapCenterY - m_fMapSize,
		m_fMapCenterX + lcsMapHalfWidth, m_fMapCenterY + m_fMapSize));
#else
...
#ifdef _3DS
	/* The controller diagram is a 2.3 MiB TXD whose native conversion needs a
	 * much larger temporary linear buffer.  It is not useful for the 3DS input
	 * layout and can exhaust a fragmented gameplay heap when pausing, so leave
	 * this optional diagram blank.  Button glyphs above remain available. */
	int frontend_controller = CTxdStore::FindTxdSlot("frontend_controller");
	if (frontend_controller != -1)
		CTxdStore::RemoveTxd(frontend_controller);
	return;
