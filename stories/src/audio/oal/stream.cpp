...
public:
...
#ifdef _3DS
			/* The 3DS streamed mix is mono.  Decode one channel directly instead of
			 * decoding stereo, sorting it and discarding half in FillBuffer. */
	...
	void FileOpen()
	{
		...
		double openStart = re3_wall_time_ms();
		...
		m_bFileNotOpenedYet = false;
		double openMs = re3_wall_time_ms() - openStart;
		if(openMs >= 40.0)
			re3_stall_log("AUDIO mp3-open %.0f ms %s", openMs, m_pPath);
	}
	...
	uint32 Decode(void *buffer)
	{
		...
		size_t size = 0;
		int err = MPG123_ERR;
		/* A format notification is not end-of-stream.  The format was pinned at
		 * open time, so consume the notification once and request audio again. */
...
#ifdef _3DS
		/* Match reVC's 3DS radio startup budget.  Half a second is enough to
		 * begin playback without making a station change prefill every buffer. */
		const int32 initialBufferPairs = m_bFullInitialQueue ?
			NUM_STREAMBUFFERS / 2 : Min(2, NUM_STREAMBUFFERS / 2);
#else
		const int32 initialBufferPairs = NUM_STREAMBUFFERS / 2;
#endif
		for(int i = 0; i < initialBufferPairs; i++) {
...
void CStream::SetPan(uint8 nPan)
{
	if (m_bSingleSource) {
		...
		SetPosition(0, x, 0.0f, Sqrt(Max(0.0f, 1.0f - SQR(x))));
		return;
	}
...
bool CStream::FillBuffer(ALuint *alBuffer)
{
	...
	if (size == 0 && m_bSeamlessLoop && m_nLoopCount != 1) {
		/* This runs on LCS's audio worker while it owns the stream. Rewind and
		 * decode into the buffer already being serviced, so the main thread never
		 * sees an empty queue between consecutive passes of finale LOOP. */
		m_pSoundFile->Seek(0);
		if (m_nLoopCount > 1)
			m_nLoopCount--;
...
#ifdef _3DS
	/* LCS streams are predominantly PS2 .VB files, so reVC's mpg123 mono flag
	 * does not reach them.  Downmix the already-decoded planar stereo PCM here:
	 * file selection, stream timing and the script-visible audio sequence remain
	 * untouched, while the 3DS output receives one mono signal. */
...
void CStream::Update()
{
	...
	if ( !m_bPaused )
	{
...
#ifdef _3DS
		/* Keep one bounded refill per frame for the 3DS mono stream.  The paired
		 * fallback remains for non-mono configurations. */
		ALint pairedBuffersProcessed = m_bSingleSource ?
			buffersProcessed[0] : Min(buffersProcessed[0], buffersProcessed[1]);
		if(pairedBuffersProcessed > 1)
			pairedBuffersProcessed = 1;
#else
		while(buffersProcessed[0] != buffersProcessed[1]){
			alGetSourcei(m_pAlSources[0], AL_BUFFERS_PROCESSED, &buffersProcessed[0]);
			alGetSourcei(m_pAlSources[1], AL_BUFFERS_PROCESSED, &buffersProcessed[1]);
		}
		ALint pairedBuffersProcessed = buffersProcessed[0];
#endif
		...
		else
		{
			...
			if (m_bActive && !tempFillBuffer.empty())
			{
#ifdef MULTITHREADED_AUDIO
				m_mutex.lock();
#endif
