...
{
#ifdef _3DS
	// Avoid a full-frame copy and colour-filter pass during ordinary gameplay.
	// The sniper overlay still uses the front buffer and remains available.
	if(EffectSwitch == POSTFX_OFF && !BlurOn && !MotionBlurOn && type != MOTION_BLUR_SNIPER)
		return;
#endif
