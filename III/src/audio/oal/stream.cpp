...
#ifdef _3DS
			/* The 3DS output mix is mono; avoid decoding an unused channel. */
...
{
#ifdef _3DS
	/* The hardware build has one mono stream source. Keep it centred instead of
	 * recreating stereo by duplicating the decoded channel into two queues. */
	m_nPan = nPan;
	SetPosition(0, 0.0f, 0.0f, 1.0f);
#else
...
bool CStream::FillBuffer(ALuint *alBuffer)
{
	...
	if (size == 0 && m_bSeamlessLoop && m_nLoopCount != 1) {
		/* Keep the decoder ahead of the hardware queue at a music loop point.
		 * Waiting for every queued block to drain leaves the 3DS restart path
		 * with only one 250 ms block, after which busy frames repeatedly starve
		 * the source. Rewind while refilling the current free block instead. */
		m_pSoundFile->Seek(0);
		if (m_nLoopCount > 1)
			m_nLoopCount--;
...
#ifdef _3DS
	/* Decoders expose stereo as two planar blocks. Mix those blocks in place so
	 * WAV/VB/Opus inputs obey the same mono contract as mpg123 MONO_MIX. */
	if(m_pSoundFile->GetChannels() == 2){
		int16 *left = (int16*)m_pBuffer;
		int16 *right = (int16*)((uint8*)m_pBuffer + channelSize);
		for(uint32 i = 0; i < channelSize / sizeof(int16); i++)
			left[i] = ((int32)left[i] + (int32)right[i]) / 2;
	}
#endif
...
void CStream::Update()
{
	...
	{
#ifdef _3DS
		ALint totalBuffers = 0;
		ALint buffersProcessed = 0;
		alGetSourcei(m_pAlSources[0], AL_BUFFERS_QUEUED, &totalBuffers);
		alGetSourcei(m_pAlSources[0], AL_BUFFERS_PROCESSED, &buffersProcessed);

		bool buffersRefilled = false;
		if (m_nLoopCount != 1 && m_bActive && totalBuffers == 0)
		{
			Setup(true);
			/* Radio recovery stays bounded to one block. Finale music is lightweight
			 * ADPCM and needs enough headroom to survive a busy script transition. */
			if(m_bMissionMusic)
				buffersRefilled = FillBuffers() != 0;
			else if(FillBuffer(&m_alBuffers[0])){
		...
		{
			/* Ordinary radio remains limited to one block. Finale ADPCM may recover
			 * two so it does not remain balanced on the edge of starvation. */
			ALint buffersToRefill = Min(buffersProcessed, m_bMissionMusic ? 2 : 1);
			while(buffersToRefill--){
				ALuint buffer;
				alSourceUnqueueBuffers(m_pAlSources[0], 1, &buffer);
				if(m_bActive && FillBuffer(&buffer)){
					alSourceQueueBuffers(m_pAlSources[0], 1, &buffer);
					buffersRefilled = true;
				}
			}
		}

		if(m_bActive && (buffersRefilled || totalBuffers - buffersProcessed > 0))
			SetPlay(true);
#else
