...
DrawSolidRocketSight(float brightness)
{
	...
	const float halfW = SCREEN_SCALE_X_PC(40.0f);
	const float halfH = SCREEN_SCALE_Y_PC(40.0f);
	const float armW = SCREEN_SCALE_X_PC(14.0f);
	const float armH = SCREEN_SCALE_Y_PC(14.0f);
...
DrawSolidStandardSight(float cx, float cy)
{
	...
	// siteM16 is a 128x128 ring whose visible edge spans about 120 pixels,
	// plus a 4x4 centre dot. Preserve those proportions at the original 0.4
	// third-person size while replacing only texture sampling.
	...
	CSprite2d::DrawRect(CRect(cx - 0.5f, cy - 0.5f, cx + 0.5f, cy + 0.5f), colour);
}
#endif
...
void CHud::Draw()
{
	...
	if (m_Wants_To_Draw_Hud && !TheCamera.m_WideScreenOn) {
...
#endif
			{
					...
					)
				{
					...
					CSprite::RenderOneXLUSprite(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 1.0f,
						SCREEN_SCALE_X_PC(40.0f), SCREEN_SCALE_Y_PC(40.0f),
						(100.0f * fMultBright), (200.0f * fMultBright), (100.0f * fMultBright), 255, 1.0f, 255);
				#endif
				}
				else {
					// Sniper
		...
		*/
		...
		const int32 wantedLevel = FindPlayerPed()->m_pWanted->GetWantedLevel();
		if(wantedLevel > 0) {
			CFont::SetBackgroundOff();
			CFont::SetScale(SCREEN_SCALE_X(0.8f), SCREEN_SCALE_Y(1.35f));
			CFont::SetJustifyOff();
			...
			// Match reVC's top-row placement: when wanted, all six stars are
			// present; earned stars use GTA3's original colour and the rest are dim.
			for(int i = 0; i < 6; i++) {
				const bool lit = wantedLevel > i &&
					(CTimer::GetTimeInMilliseconds() >
					 FindPlayerPed()->m_pWanted->m_nLastWantedLevelChange + 2000 ||
					 (CTimer::GetFrameCounter() & 4));
				CFont::SetColor(lit ? WANTED_COLOR : CRGBA(70, 60, 44, 220));
				CFont::PrintString(SCREEN_SCALE_FROM_RIGHT(24.0f + 23.0f * i),
					SCREEN_SCALE_Y(28.0f), sPrintIcon);
			}
		}
		#else
		CFont::SetBackgroundOff();
		...
		if (m_PagerMessage[0] || PagerOn == 2) {
			...
			if (PagerOn == 1) {
				if (PagerXOffset > 0.0f) {
					...
					if (fStep > 10.0f)
						fStep = 10.0f;
					PagerXOffset -= fStep * HudDisplayTimer::GetTimeStep();
				}
				if (!PagerSoundPlayed) {
		...
		if (m_BigMessage[0][0]) {
			if (BigMessageInUse[0] != 0.0f) {
...
#endif
				{
					...
					BigMessageX[0] += SCREEN_SCALE_X_FIX(HudDisplayTimer::GetTimeStepInMilliseconds() * 0.3f);
					BigMessageAlpha[0] += (HudDisplayTimer::GetTimeStepInMilliseconds() * 0.3f);
...
void CHud::DrawAfterFade()
{
	...
	if (m_HelpMessage[0]) {
		...
		if (m_HelpMessageState != 0) {
			...
			CFont::SetColor(CRGBA(175, 175, 175, 255));
#ifdef _3DS
			if(!ProfileHintLayout3DS::Draw(m_HelpMessageToPrint, SCREEN_SCALE_X(26.0f), SCREEN_SCALE_Y(28.0f) + SCREEN_SCALE_Y_FIX((150.0f - PagerXOffset) * 0.6f),
	...
	if (m_BigMessage[1][0]) {
		if (BigMessageInUse[1] != 0.0f) {
			...
			if (BigMessageX[1] >= SCREEN_WIDTH - SCREEN_SCALE_X_FIX(20.0f))
			{
				...
				BigMessageX[1] += SCREEN_SCALE_X_FIX(HudDisplayTimer::GetTimeStepInMilliseconds() * 0.3f);
				BigMessageAlpha[1] += (HudDisplayTimer::GetTimeStepInMilliseconds() * 0.3f);
