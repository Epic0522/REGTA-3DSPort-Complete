...
static void
Skip3DSTouchButtonWord(const wchar *str, uint16 strSize, uint16 &i, wchar *&out)
{
	uint16 j = i;
	while (j < strSize && str[j] == ' ')
		j++;
	uint16 formatAt = j;
	bool hasFormat = j + 2 < strSize && str[j] == '~' && str[j + 2] == '~';
	if (hasFormat) {
		j += 3;
		while (j < strSize && str[j] == ' ')
			j++;
	...
	for (uint16 k = 0; k < sizeof(button) - 1; k++)
		if (j + k >= strSize || str[j + k] != button[k])
			return;
	if (hasFormat)
...
#ifdef _3DS
						const bool touchInstruction = keybuf_size != 0 && Is3DSTouchInstruction(keybuf);
						if (touchInstruction)
...
#ifdef _3DS
						if (touchInstruction)
							Skip3DSTouchButtonWord(str, strSize, i, _outstr);
#endif
					}
				}
			}
