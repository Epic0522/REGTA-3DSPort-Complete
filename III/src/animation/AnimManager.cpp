...
#ifdef _3DS
// PED.IFP contains thousands of tiny keyframe records. Reading each one
// through stdio separately is disproportionately expensive on a 3DS SD card,
// so the filename entry point may temporarily feed the parser from one bulk
// allocation. Direct-fd callers (notably CUTS.IMG) keep their original path.
static const uint8 *gAnimPreloadData;
static uint32 gAnimPreloadSize;
static uint32 gAnimPreloadPos;

static size_t
AnimFileRead(int fd, void *buffer, size_t size)
{
	if(gAnimPreloadData == nil)
		return CFileMgr::Read(fd, (char*)buffer, size);
	uint32 available = gAnimPreloadPos < gAnimPreloadSize ?
		gAnimPreloadSize - gAnimPreloadPos : 0;
	if(size > available)
		size = available;
	memcpy(buffer, gAnimPreloadData + gAnimPreloadPos, size);
	gAnimPreloadPos += size;
	return size;
}

static void
AnimFileSeek(int fd, int offset, int whence)
{
	if(gAnimPreloadData == nil) {
		CFileMgr::Seek(fd, offset, whence);
		return;
	}
	int64 base = whence == SEEK_SET ? 0 :
		whence == SEEK_CUR ? gAnimPreloadPos : gAnimPreloadSize;
	int64 position = base + offset;
	gAnimPreloadPos = clamp(position, (int64)0, (int64)gAnimPreloadSize);
}
#else
static size_t AnimFileRead(int fd, void *buffer, size_t size)
{
	return CFileMgr::Read(fd, (char*)buffer, size);
}
static void AnimFileSeek(int fd, int offset, int whence)
{
	CFileMgr::Seek(fd, offset, whence);
}
#endif
...
#ifdef _3DS
	// Only filename-based standalone IFPs use the bulk path. LoadAnimFile(fd)
	// remains untouched for slices inside CUTS.IMG.
	long start = CFileMgr::Tell(fd);
	CFileMgr::Seek(fd, 0, SEEK_END);
	long end = CFileMgr::Tell(fd);
	CFileMgr::Seek(fd, start, SEEK_SET);
	uint8 *preload = nil;
	if(start >= 0 && end > start) {
		uint32 size = end - start;
		preload = (uint8*)RwMalloc(size);
		if(preload && CFileMgr::Read(fd, (char*)preload, size) == size) {
			gAnimPreloadData = preload;
			gAnimPreloadSize = size;
			gAnimPreloadPos = 0;
		} else {
			RwFree(preload);
			preload = nil;
...
CAnimManager::LoadAnimFile(int fd, bool compress)
{
	...
	AnimFileRead(fd, &anpk, sizeof(IfpHeader));
	if(!CGeneral::faststrncmp(anpk.ident, "ANLF", 4)) {
		ROUNDSIZE(anpk.size);
		AnimFileRead(fd, buf, anpk.size);
		numANPK = *(int*)buf;
	} else if(!CGeneral::faststrncmp(anpk.ident, "ANPK", 4)) {
		AnimFileSeek(fd, -8, SEEK_CUR);
		numANPK = 1;
	}
	...
	for(i = 0; i < numANPK; i++){
		...
		AnimFileRead(fd, &anpk, sizeof(IfpHeader));
		ROUNDSIZE(anpk.size);
		AnimFileRead(fd, &info, sizeof(IfpHeader));
		ROUNDSIZE(info.size);
		AnimFileRead(fd, buf, info.size);
		...
		for(j = 0; j < animBlock->numAnims; j++){
			...
			// animation name
			AnimFileRead(fd, &name, sizeof(IfpHeader));
			ROUNDSIZE(name.size);
			AnimFileRead(fd, buf, name.size);
			hier->SetName(buf);

			// DG info has number of nodes/sequences
			AnimFileRead(fd, &dgan, sizeof(IfpHeader));
			ROUNDSIZE(dgan.size);
			AnimFileRead(fd, &info, sizeof(IfpHeader));
			ROUNDSIZE(info.size);
			AnimFileRead(fd, buf, info.size);
			...
			for(k = 0; k < hier->numSequences; k++, seq++){
				...
				AnimFileRead(fd, &cpan, sizeof(IfpHeader));
				ROUNDSIZE(dgan.size);
				AnimFileRead(fd, &anim, sizeof(IfpHeader));
				ROUNDSIZE(anim.size);
				AnimFileRead(fd, buf, anim.size);
				...
				AnimFileRead(fd, &info, sizeof(info));
