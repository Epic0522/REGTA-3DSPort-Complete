...
EncodeSaveNameField(wchar *field, const wchar *title, const char *gxtKey)
{
	...
	/* Keep the original 48-byte save header and all following offsets intact.
	 * Long names retain a short legacy preview, while current builds resolve the
	 * embedded GXT key into the complete title in the selected language. */
...
DecodeSaveNameField(wchar *name, const wchar *field)
{
	...
	/* Recover complete titles from saves written by older builds, whose header
	 * kept only the first 18 characters followed by an ellipsis. */
	if(field[18] == '.' && field[19] == '.' && field[20] == '.'){
		static const char *keys[] = { "JM2", "JM5", "TM1", "TM3", "TM4" };
